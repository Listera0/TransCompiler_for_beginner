...
BaseRule    = ["bool", "string"]
JavaRule    = ["boolean", "String"]
...
## 코드 변환 base -> Java
def ConvertCodeToJava( container, depth ):
    result = container
...
def ConvertCodeToBase( container ):
    ...
    for i in range(0, len(container.Announces)):
        if result.Announces[i].AccessModifier in JavaRule:
            result.Announces[i].AccessModifier = BaseRule[JavaRule.index(result.Announces[i].AccessModifier)]
        elif result.Announces[i].StaticModifier in JavaRule:
            result.Announces[i].StaticModifier = BaseRule[JavaRule.index(result.Announces[i].StaticModifier)]
        elif result.Announces[i].DataType in JavaRule:
            result.Announces[i].DataType = BaseRule[JavaRule.index(result.Announces[i].DataType)]
    ...
    for i in range(0, len(code)):
        ...
        if i >= finishindex:
            ...
            if isword and code[i] in WordEndList:
                ...
                if word not in JavaAccessModifier and word not in JavaDataType and word != "static":
                    ## 선언문의 경우
...
def ConvertAnnounce(code):
    ...
    for i in range(0, len(code)):
        ...
        if startword and code[i] in WordEndList:
            ...
            if word in JavaAccessModifier:
                result.AccessModifier = word
            elif word == "static":
                result.StaticModifier = word
            elif word in JavaDataType:
