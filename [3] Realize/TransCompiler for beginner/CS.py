...
CSDataType = ["int", "string", "long", "char", "bool", "double", "short", "float"]
CSStatic = ["static"]
    ...
    for i in range(0, len(code.IndexList)):
        if code.IndexList[i] == "Announce":
            ...
            for j in range(0, len(code.IfMethods[ifC].Condition)):
                ...
                temp += ConvertCodeToCS(code.IfMethods[ifC].Value[j], depth)
                depth -= 1
                temp += IndentSpace(depth) + "}\n"
            ...
            if code.IfMethods[ifC].Else != None:
                ...
                temp += ConvertCodeToCS(code.IfMethods[ifC].Else, depth) + "\n"
                depth -= 1
                temp += IndentSpace(depth) + "}\n"
            ...
            temp += ConvertCodeToCS(code.ForMethods[forC].Value, depth)
            depth -= 1
            temp += IndentSpace(depth) + "}\n"
            ...
            temp += ConvertCodeToCS(code.WhileMethods[whileC].Value, depth)
            depth -= 1
            temp += IndentSpace(depth) + "}\n"
