...
## 코드 변환 base -> C
def ConvertCodeToC( container, depth ):
    result = container
    
    for i in range(0, len(container.Announces)):
        if result.Announces[i].AccessModifier in BaseRule:
            result.Announces[i].AccessModifier = CRule[BaseRule.index(result.Announces[i].AccessModifier)]
        elif result.Announces[i].StaticModifier in BaseRule:
            result.Announces[i].StaticModifier = CRule[BaseRule.index(result.Announces[i].StaticModifier)]
        elif result.Announces[i].DataType in BaseRule:
            result.Announces[i].DataType = CRule[BaseRule.index(result.Announces[i].DataType)]
...
def ConvertCodeToBase( container ):
    ...
    for i in range(0, len(container.Announces)):
        if result.Announces[i].AccessModifier in CRule:
            result.Announces[i].AccessModifier = BaseRule[CRule.index(result.Announces[i].AccessModifier)]
        elif result.Announces[i].StaticModifier in CRule:
            result.Announces[i].StaticModifier = BaseRule[CRule.index(result.Announces[i].StaticModifier)]
        elif result.Announces[i].DataType in CRule:
            result.Announces[i].DataType = BaseRule[CRule.index(result.Announces[i].DataType)]
    ...
    for i in range(0, len(code.IndexList)):
        if code.IndexList[i] == "Announce":
            ...
            for j in range(0, len(code.IfMethods[ifC].Condition)):
                ...
                temp += ConvertCodeToC(code.IfMethods[ifC].Value[j], depth)
                depth -= 1
                temp += IndentSpace(depth) + "}\n"
            ...
            if code.IfMethods[ifC].Else != None:
                ...
                temp += ConvertCodeToC(code.IfMethods[ifC].Else, depth) + "\n"
                depth -= 1
                temp += IndentSpace(depth) + "}\n"
            ...
            temp += "for(" + ConvertCodeToC(tempCon, 0).replace('\n', '') + " " 
            ...
            tempCon.UsingName += code.UsingName
            temp += ConvertCodeToC(tempCon, 0).replace('\n', '').rstrip(';') + ")\n"
            ...
            temp += ConvertCodeToC(code.ForMethods[forC].Value, depth)
            depth -= 1
            temp += IndentSpace(depth) + "}\n"
            ...
            temp += ConvertCodeToC(code.WhileMethods[whileC].Value, depth)
            depth -= 1
            temp += IndentSpace(depth) + "}\n"
    ...
    for i in range(0, len(code)):
        ...
        if i >= finishindex:
            ...
            if isword and code[i] in WordEndList:
                ...
                if word not in CAccessModifier and word not in CDataType and word != "static":
                    ## 선언문의 경우
