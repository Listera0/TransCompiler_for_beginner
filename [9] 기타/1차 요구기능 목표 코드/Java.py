...
def CombineCode( code, depth ):
    ...
    for i in range(0, len(code.Conditions)):
        temp = ""
        temp += code.Conditions[i].Target + " " + code.Conditions[i].Operator + " " + code.Conditions[i].Value
        result += temp
    ...
    for i in range(0, len(code.IfMethods)):
        ...
        for j in range(0, len(code.IfMethods[i].Condition)):
            ...
            tempcontainer.Conditions.append(code.IfMethods[i].Condition[j])
            ...
            temp += "if (" + CombineCode(tempcontainer, depth) + ")\n"
